...
- **Power Node wireless links** — nodes now link to power structures within 6 blocks instead of needing every hop to touch: place a node and it auto-connects to the nearest valid structures (up to 3 links each), right-click a node then another to link or unlink them manually, and double-click a node to auto-fill or clear its links. A link is a real connection (breaking a node splits its two grid halves), is blocked by full terrain walls, e.g. stone, in a straight line, and survives save/load. Each node also draws the links as glowing beams whose color telegraphs grid health — white when fully powered, amber when the grid is browning out.
